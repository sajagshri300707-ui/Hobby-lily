...
export default function SignupPage() {
  ...
  const { signup, signupWithPhone, loginWithGoogle } = useAuth();
  ...
  const handleSubmit = async (e) => {
    ...
    if (authMode === 'email') {
      ...
      if (!otpSent) {
        ...
        setLoading(true);
        try {
          await signupWithPhone(name, phone, otp);
          navigate('/app/garden');
        } catch (err) {
      ...
      >
        <div style={{ maxWidth: '420px', width: '100%', margin: '0 auto' }}>
          ...
          <p style={{ color: 'var(--ink-muted)', fontFamily: 'var(--font-body)', marginBottom: '2rem' }}>
            {t('signup.alreadyGrowing')}{' '}
            <Link to="/login" style={{ color: 'var(--brown-coffee)', fontWeight: 600, textDecoration: 'none' }}>{t('signup.signIn')}</Link>
          </p>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.1rem' }}>
            <div>
              ...
              <input id="signup-name" className="form-input" type="text" placeholder={t('signup.namePlaceholder')} value={name} onChange={e => setName(e.target.value)} autoComplete="name" />
            </div>
            {authMode === 'email' ? (
              <>
                <div>
                  <label className="form-label">{t('signup.email')}</label>
                  <input id="signup-email" className="form-input" type="email" placeholder="you@example.com" value={email} onChange={e => setEmail(e.target.value)} autoComplete="email" />
                </div>
                <div>
                  <label className="form-label">{t('signup.password')}</label>
                  <input id="signup-password" className="form-input" type="password" placeholder={t('signup.passwordHint')} value={password} onChange={e => setPassword(e.target.value)} autoComplete="new-password" />
                </div>
              </>
            ) : (
              <>
                <div>
                  ...
                  <input id="signup-phone" className="form-input" type="tel" placeholder="(555) 123-4567" value={phone} onChange={e => setPhone(e.target.value)} disabled={otpSent} />
                </div>
                {otpSent && (
                  <div>
                    <label className="form-label">OTP Code</label>
                    <input id="signup-otp" className="form-input" type="text" placeholder="123456" value={otp} onChange={e => setOtp(e.target.value)} />
                  </div>
                )}
              </>
            )}
            <button id="signup-submit" type="submit" className="btn btn-primary" disabled={loading} style={{ marginTop: '0.5rem', width: '100%', justifyContent: 'center' }}>
              {loading ? <><span className="loading-dot"/><span className="loading-dot"/><span className="loading-dot"/></> : (authMode === 'phone' && !otpSent ? 'Send OTP' : t('signup.submit'))}
            </button>
          ...
          <button type="button" onClick={handleGoogleSignup} className="btn btn-ghost" style={{ width: '100%', justifyContent: 'center', gap: '10px' }}>
